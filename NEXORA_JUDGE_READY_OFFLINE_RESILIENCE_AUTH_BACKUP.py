...
def auth_css():
    st.markdown(r"""
    <style>
    .auth-wrap{max-width:560px;margin:7vh auto 0;padding:0 14px}
    .auth-card{background:linear-gradient(145deg,rgba(8,14,39,.96),rgba(22,7,48,.94));border:1px solid rgba(83,190,255,.32);border-radius:24px;padding:34px;box-shadow:0 0 55px rgba(38,119,255,.16),inset 0 1px 0 rgba(255,255,255,.05)}
    .auth-brand{font-family:Orbitron,sans-serif;font-weight:900;letter-spacing:3px;font-size:30px;color:#f2f7ff;text-align:center}
    .auth-brand span{color:#39d5ff}.auth-kicker{text-align:center;color:#7e99c3;font-size:10px;letter-spacing:2px;margin:8px 0 26px}
    .auth-step{display:flex;gap:8px;justify-content:center;margin:0 0 22px;flex-wrap:wrap}.auth-step span{font-size:9px;padding:7px 10px;border-radius:999px;border:1px solid rgba(93,167,255,.25);color:#8da8d1;background:rgba(39,101,210,.09)}.auth-step .active{color:#eaf8ff;border-color:rgba(56,215,255,.65);background:rgba(30,166,255,.14)}
    .auth-note{padding:12px 14px;border-radius:12px;background:rgba(47,111,255,.08);border:1px solid rgba(76,164,255,.18);color:#91a8ca;font-size:10px;line-height:1.55;margin-top:14px}
    .auth-success{padding:14px;border-radius:12px;background:rgba(37,215,164,.08);border:1px solid rgba(53,240,176,.28);color:#7ef2ca;font-size:11px}
    .auth-email{color:#39d5ff;font-weight:800}
    </style>
...
def render_auth_gate():
    ...
    st.markdown('<div class="auth-brand">NEXORA<span>•</span>SECURE</div>', unsafe_allow_html=True)
    st.markdown('<div class="auth-kicker">PREDICTIVE RUNTIME ORCHESTRATION · IDENTITY GATE</div>', unsafe_allow_html=True)

    if mode == "login":
        st.markdown('<div class="auth-step"><span class="active">1 PASSWORD</span><span>2 EMAIL OTP</span><span>3 WORKSPACE</span></div>', unsafe_allow_html=True)
        ...
        with st.form("nexora_login_form"):
            ...
            submitted = st.form_submit_button("CONTINUE TO EMAIL OTP", use_container_width=True)
        if submitted:
            ...
            if user is None or not verify_password(password, user["password_hash"]):
                ...
                try:
                    ...
                    st.error(str(exc))
        st.markdown('<div class="auth-note">Your password is checked locally. The NEXORA workspace is unlocked only after the one-time code sent to your registered email is verified.</div>', unsafe_allow_html=True)
        if st.button("CREATE NEXORA ACCOUNT", use_container_width=True):
            st.session_state.auth_mode = "register"
            st.rerun()

    elif mode == "register":
        st.markdown('<div class="auth-step"><span class="active">CREATE ACCOUNT</span><span>EMAIL OTP</span><span>WORKSPACE</span></div>', unsafe_allow_html=True)
        st.subheader("Create NEXORA Account")
        with st.form("nexora_register_form"):
            email = st.text_input("Your actual email address", placeholder="you@gmail.com")
            ...
            if "@" not in email or "." not in email.split("@")[-1]:
                ...
                if existing:
                    ...
                    try:
                        ...
                        # Remove the account if the required email verification could not be sent.
                        conn = auth_db()
        ...
        st.markdown('<div class="auth-step"><span>1 PASSWORD</span><span class="active">2 EMAIL OTP</span><span>3 WORKSPACE</span></div>', unsafe_allow_html=True)
        st.subheader("Verify Your Email")
        st.markdown(f'<div class="auth-note">A 6-digit OTP was sent to <span class="auth-email">{esc(email)}</span>. It expires in 5 minutes.</div>', unsafe_allow_html=True)
        with st.form("nexora_otp_form"):
            code = st.text_input("6-digit verification code", max_chars=6, placeholder="123456")
            submitted = st.form_submit_button("VERIFY OTP & OPEN NEXORA", use_container_width=True)
        if submitted:
            if not user_id:
                st.session_state.auth_mode = "login"
                st.rerun()
