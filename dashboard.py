...
.bench-hero{padding:25px 28px;border:1px solid rgba(105,220,255,.28);border-radius:22px;background:radial-gradient(circle at 8% 20%,rgba(46,150,255,.18),transparent 32%),radial-gradient(circle at 90% 80%,rgba(173,76,255,.16),transparent 35%),rgba(6,12,34,.92);margin-bottom:22px;box-shadow:0 18px 45px rgba(0,0,0,.20)}
.bench-hero-kicker{font:900 10px Orbitron;letter-spacing:1.8px;color:#69ddff}.bench-hero-title{font:900 26px Orbitron;color:#f3f7ff;margin-top:7px}.bench-hero-sub{max-width:1050px;color:#9aafd0;font-size:14px;line-height:1.65;margin-top:9px}.bench-hero-sub b{color:#dceaff}
.bench-note{margin:12px 0 20px;padding:14px 17px;border-left:3px solid #a966ff;border-radius:12px;background:rgba(130,65,220,.08);color:#aebed8;font-size:12px;line-height:1.6}.bench-note b{color:#eef6ff}
.bench-empty{display:flex;gap:15px;align-items:center;margin-top:20px;padding:24px;border:1px dashed rgba(105,220,255,.28);border-radius:18px;background:rgba(6,13,34,.65)}.bench-empty-icon{width:48px;height:48px;border-radius:14px;display:flex;align-items:center;justify-content:center;background:rgba(73,220,255,.10);border:1px solid rgba(73,220,255,.25);font:900 11px Orbitron;color:#69ddff}.bench-empty b{display:block;color:#edf5ff;font:900 13px Orbitron}.bench-empty span{display:block;color:#8499bc;font-size:12px;line-height:1.5;margin-top:5px}
.bench-understood{display:grid;grid-template-columns:1fr 2.3fr 1fr 1fr;gap:10px;margin-bottom:20px}.bench-understood>div{padding:16px;border:1px solid rgba(95,132,209,.20);border-radius:14px;background:rgba(6,13,34,.80);min-height:78px}.bench-understood span{display:block;color:#6f87ae;font:900 9px Orbitron;letter-spacing:1px}.bench-understood b{display:block;color:#eaf3ff;font-size:13px;line-height:1.45;margin-top:8px;word-break:break-word}
.bench-metrics-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin:12px 0 16px}.bench-metric{padding:19px;border-radius:16px;border:1px solid rgba(96,134,212,.22);background:linear-gradient(145deg,rgba(9,18,46,.96),rgba(5,10,29,.96));min-height:128px}.bench-label{font:900 9px Orbitron;color:#7892b8;letter-spacing:1px}.bench-value{font:900 28px Orbitron;color:#eef6ff;margin-top:10px}.bench-sub{font-size:11px;color:#8499bc;margin-top:6px}
.bench-verdict{padding:21px;text-align:center;border-radius:18px;border:1px solid rgba(105,220,255,.25);background:linear-gradient(100deg,rgba(25,77,160,.12),rgba(120,55,190,.12));margin:8px 0 24px}.bench-verdict-label{font:900 10px Orbitron;color:#8fa8cc;letter-spacing:1.3px}.bench-verdict-number{font:900 42px Orbitron;color:#69ddff;margin:6px 0;text-shadow:0 0 25px rgba(105,220,255,.22)}.bench-verdict-text{font-size:11px;color:#899fbe;line-height:1.5;max-width:850px;margin:auto}
.bench-side{padding:21px;border-radius:18px;min-height:175px;border:1px solid rgba(100,135,225,.22);background:rgba(7,14,37,.88)}.bench-side.reactive{border-top:3px solid #ffd766}.bench-side.nexora{border-top:3px solid #42efb4}.bench-side-kicker{font:900 9px Orbitron;color:#7892b8;letter-spacing:1.3px}.bench-side-title{font:900 18px Orbitron;color:#f1f7ff;margin-top:6px}.bench-side-flow{margin-top:15px;padding:12px;border-radius:11px;background:rgba(6,12,31,.8);color:#c8d7ed;font-size:12px;line-height:1.6}.bench-side-detail{color:#8197ba;font-size:11px;line-height:1.5;margin-top:10px}
.bench-trace-title{font:900 10px Orbitron;color:#69ddff;letter-spacing:1px;margin:5px 0 8px}.bench-trace-title+div pre{min-height:280px}
@media(max-width:1000px){.bench-understood{grid-template-columns:repeat(2,1fr)}.bench-metrics-grid{grid-template-columns:repeat(2,1fr)}}
@media(max-width:650px){.bench-understood,.bench-metrics-grid{grid-template-columns:1fr}.bench-hero-title{font-size:21px}.bench-verdict-number{font-size:34px}}
...
def _benchmark_plan_from_ai(task):
    """Ask the real local Ollama model to create a task-specific service graph."""
    ai = LocalAI()
    prompt = f"""
You are the planning agent inside a runtime benchmark.

USER TASK:
{task}

Convert the task into the smallest sensible ordered service workflow.
Use ONLY these service names:
SEARCH, PYTHON, DATABASE, LLM, VECTOR_DB, EMBEDDING, API, OCR, VISION_MODEL, IMAGE_GENERATION

Rules:
- Include a service only if the task actually needs that capability.
- Preserve the logical dependency order.
- A task may use 1 to 8 services.
- Do not invent services that are unrelated to the task.
- If the task asks to research/current information, use SEARCH.
- If it asks for arithmetic/calculation, use PYTHON.
- If it asks to retrieve/store/query structured system data, use DATABASE.
- If it asks for image creation, use IMAGE_GENERATION.
- If it asks for image understanding/OCR, use VISION_MODEL or OCR as appropriate.
- If it asks for embeddings/vector retrieval, use EMBEDDING and/or VECTOR_DB.
- Text reasoning/report generation can use LLM.

Return ONLY JSON:
{{"workflow":["SERVICE1","SERVICE2"]}}
"""
    raw = ai.generate(prompt, temperature=0.0).strip()
    cleaned = re.sub(r"```(?:json)?|```", "", raw, flags=re.I).strip()
    match = re.search(r"\{.*\}", cleaned, flags=re.S)
    if not match:
        raise RuntimeError(f"Ollama did not return a workflow JSON:\n{raw}")

    data = json.loads(match.group(0))
    allowed = {
        "SEARCH", "PYTHON", "DATABASE", "LLM", "VECTOR_DB",
        "EMBEDDING", "API", "OCR", "VISION_MODEL", "IMAGE_GENERATION"
    }
    workflow = [str(x).upper().strip() for x in data.get("workflow", [])]
    workflow = [x for x in workflow if x in allowed]

    if not workflow:
        raise RuntimeError(
            "The local AI returned an empty workflow for this task. "
            f"AI response:\n{raw}"
        )

    # Remove accidental duplicate adjacent services while preserving real repeats.
    compact = []
    for service in workflow:
        if not compact or compact[-1] != service:
            compact.append(service)

    return compact, raw


def _benchmark_service_duration(service):
    """Small deterministic runtime costs used only for the orchestration benchmark."""
    return {
        "SEARCH": 0.055,
        "PYTHON": 0.035,
        "DATABASE": 0.075,
...
def _benchmark_competitor_duration(service):
    return _benchmark_service_duration(service) * 3.0


def _execute_ai_reactive(workflow):
    """Baseline: same AI workflow, but no look-ahead reservation."""
    start = time.perf_counter()
    locks = {service: threading.Lock() for service in set(workflow)}
    trace = []
    wait_total = 0.0
    wait_by_service = {}

    # Choose the first shared service as the contention point. This is derived
    # from the AI-generated workflow, so different tasks produce different runs.
    contention = workflow[1] if len(workflow) > 1 else workflow[0]

    def competitor():
        # The competitor starts before the AI reaches the contention point.
        time.sleep(0.018)
        trace.append(f"COMPETING WORKFLOW → requests {contention}")
        with locks[contention]:
            trace.append(f"COMPETING WORKFLOW → occupies {contention}")
            time.sleep(_benchmark_competitor_duration(contention))
        trace.append(f"COMPETING WORKFLOW → releases {contention}")

    def ai_agent():
        nonlocal wait_total
        for index, service in enumerate(workflow):
            if service == contention:
                trace.append(
                    f"AI AGENT → step {index + 1}: requests {service}"
                )
                request = time.perf_counter()
                with locks[service]:
                    waited = time.perf_counter() - request
                    wait_total += waited
                    wait_by_service[service] = waited
                    trace.append(
                        f"AI AGENT → {service} allocated after {waited:.3f}s wait"
                    )
                    time.sleep(_benchmark_service_duration(service))
                trace.append(f"AI AGENT → {service} released")
            else:
                time.sleep(_benchmark_service_duration(service))
                trace.append(
                    f"AI AGENT → step {index + 1}: {service} executed"
                )

    competitor_thread = threading.Thread(target=competitor)
    ai_thread = threading.Thread(target=ai_agent)
    ...
    return {
        ...
        "wait": wait_total,
        "wait_by_service": wait_by_service,
        "trace": trace,
        "contention": contention,
    }


def _execute_ai_with_nexora(workflow):
    """Same AI workflow, with NEXORA predicting and reserving the next dependency."""
    start = time.perf_counter()
    locks = {service: threading.Lock() for service in set(workflow)}
    reservations = {service: threading.Lock() for service in set(workflow)}
    trace = []
    wait_total = 0.0
    wait_by_service = {}
    ...
    # NEXORA looks ahead from the current workflow and reserves the future
    # dependency before the competitor can consume it.
    reservation_taken = reservations[contention].acquire(blocking=False)
    if reservation_taken:
        trace.append(
            f"NEXORA PREDICTION → next constrained dependency = {contention}"
        )
        trace.append(
            f"NEXORA RESERVATION → {contention} capacity protected"
        )

    def competitor():
        time.sleep(0.018)
        trace.append(f"COMPETING WORKFLOW → requests {contention}")
        while reservations[contention].locked():
            time.sleep(0.003)
        with locks[contention]:
            trace.append(
                f"COMPETING WORKFLOW → occupies {contention} after NEXORA release"
            )
            time.sleep(_benchmark_competitor_duration(contention))
        trace.append(f"COMPETING WORKFLOW → releases {contention}")
    ...
    def ai_agent():
        ...
        for index, service in enumerate(workflow):
            if service == contention:
                trace.append(
                    f"AI AGENT + NEXORA → step {index + 1}: requests {service}"
                )
                request = time.perf_counter()
                with locks[service]:
                    waited = time.perf_counter() - request
                    ...
                    wait_by_service[service] = waited
                    trace.append(
                        f"NEXORA ALLOCATION → {service} from protected reservation"
                    )
                    time.sleep(_benchmark_service_duration(service))
                if reservation_taken and reservations[service].locked():
                    reservations[service].release()
                trace.append(f"NEXORA RELEASE → {service}")
            else:
                time.sleep(_benchmark_service_duration(service))
                trace.append(
                    f"AI AGENT + NEXORA → step {index + 1}: {service} executed"
                )
    ...
    if reservation_taken and reservations[contention].locked():
        reservations[contention].release()
...
def run_ai_vs_nexora_benchmark(task, trials=3):
    """Run a fresh task-specific A/B benchmark using the real local AI planner."""
    if not task or not task.strip():
        raise ValueError("Enter a problem statement before running the benchmark.")

    rows = []
    for trial in range(1, trials + 1):
        workflow, raw = _benchmark_plan_from_ai(task)
        reactive = _execute_ai_reactive(workflow)
        nexora = _execute_ai_with_nexora(workflow)

        rows.append({
            "trial": trial,
            "workflow": workflow,
            "raw_ai": raw,
            "ai_total": reactive["total"],
            "nexora_total": nexora["total"],
            "ai_wait": reactive["wait"],
            "nexora_wait": nexora["wait"],
            "contention": reactive["contention"],
            "ai_trace": reactive["trace"],
            "nexora_trace": nexora["trace"],
        })

    def avg(key):
        return sum(row[key] for row in rows) / len(rows)

    ai_total = avg("ai_total")
    nexora_total = avg("nexora_total")
    ai_wait = avg("ai_wait")
    nexora_wait = avg("nexora_wait")
    reduction = ((ai_total - nexora_total) / ai_total * 100.0) if ai_total else 0.0

    return {
        "mode": "REAL_LOCAL_AI_TASK_SPECIFIC",
        "model": LocalAI().model,
        "task": task,
        "trials": trials,
        "rows": rows,
...
def _benchmark_metric_card(label, value, sub=""):
    return (
        f'<div class="bench-metric">'
...
def render_benchmark_section():
    render_section_header(
        "REAL AI vs NEXORA",
        "Same local AI. Same problem statement. Different runtime orchestration.",
        "TASK-SPECIFIC A/B TEST"
    )
    ...
    st.markdown(
        ...
        '<div class="bench-hero-kicker">CONTROLLED RUNTIME COMPARISON</div>'
        '<div class="bench-hero-title">Does NEXORA change how the same AI executes?</div>'
        '<div class="bench-hero-sub">'
        'Ollama <b>llama3.2:3b</b> first converts your problem statement into a task-specific '
        'service workflow. That exact workflow is then executed under two policies: '
        '<b>Reactive AI</b> and <b>AI + NEXORA</b>. The benchmark is rebuilt from the task every time.'
        '</div>'
        '</div>',
        unsafe_allow_html=True
    )

    st.markdown("### 01 · Problem statement under test")
    benchmark_task = st.text_area(
        ...
        label_visibility="collapsed",
        placeholder="Enter a different problem statement here and run the benchmark again..."
    )

    st.markdown(
        '<div class="bench-note">'
        '<b>Important:</b> The previous benchmark used one hard-coded task and forced the AI into the same '
        'four-service workflow. That is why different problem statements produced essentially the same result. '
        'This version removes that fixed workload: the AI generates the workflow from <i>your actual input</i>.'
        '</div>',
        unsafe_allow_html=True
    )

    if st.button("▶ RUN FRESH AI vs NEXORA BENCHMARK", use_container_width=True, type="primary"):
        try:
            with st.spinner("Ollama is reading your problem statement and both runtimes are executing the resulting workflow..."):
    ...
    if not benchmark or benchmark.get("mode") != "REAL_LOCAL_AI_TASK_SPECIFIC":
        st.markdown(
            '<div class="bench-empty">'
            '<div class="bench-empty-icon">AI</div>'
            '<div><b>READY FOR A REAL COMPARISON</b>'
            '<span>Enter a problem statement and run the benchmark. Change the task and the generated service graph, contention resource, traces and measured timings can change with it.</span></div>'
            '</div>',
            unsafe_allow_html=True
        )
        return
    ...
    st.markdown("### 02 · What the AI actually understood")
    st.markdown(
        '<div class="bench-understood">'
        f'<div><span>MODEL</span><b>{esc(benchmark["model"])}</b></div>'
        f'<div><span>WORKFLOW</span><b>{esc(" → ".join(workflow))}</b></div>'
        f'<div><span>CONTENTION POINT</span><b>{esc(first["contention"])}</b></div>'
        f'<div><span>TRIALS</span><b>{benchmark["trials"]} fresh runs</b></div>'
        '</div>',
        unsafe_allow_html=True
    )

    st.markdown("### 03 · Measured runtime")
    ai_total = benchmark["ai_total"]
    nx_total = benchmark["nexora_total"]
    ai_wait = benchmark["ai_wait"]
    nx_wait = benchmark["nexora_wait"]
    reduction = benchmark["latency_reduction"]

    st.markdown(
        '<div class="bench-metrics-grid">'
        + _benchmark_metric_card("REACTIVE AI", f"{ai_total:.3f}s", "same AI · no prediction")
        + _benchmark_metric_card("AI + NEXORA", f"{nx_total:.3f}s", "same AI · predictive reservation")
        + _benchmark_metric_card("AI WAIT", f"{ai_wait:.3f}s", "resource waiting")
        + _benchmark_metric_card("NEXORA WAIT", f"{nx_wait:.3f}s", "resource waiting")
        + '</div>',
        unsafe_allow_html=True
    )

    st.markdown(
        f'<div class="bench-verdict">'
        f'<div class="bench-verdict-label">MEASURED WORKFLOW LATENCY DIFFERENCE</div>'
        f'<div class="bench-verdict-number">{reduction:.1f}%</div>'
        f'<div class="bench-verdict-text">'
        f'Computed from {benchmark["trials"]} fresh executions of the task-specific workflow. '
        f'This measures orchestration/resource waiting; it does not claim that NEXORA makes Ollama token generation itself faster.'
        f'</div></div>',
        unsafe_allow_html=True
    )

    st.markdown("### 04 · Side-by-side execution model")
    left, right = st.columns(2)
    with left:
        st.markdown(
            '<div class="bench-side reactive">'
            '<div class="bench-side-kicker">BASELINE</div>'
            '<div class="bench-side-title">REACTIVE AI</div>'
            '<div class="bench-side-flow">AI gets current step → requests resource → discovers contention → waits → continues</div>'
            '<div class="bench-side-detail">The AI does not protect a future resource before it reaches that step.</div>'
            '</div>', unsafe_allow_html=True
        )
    with right:
        st.markdown(
            '<div class="bench-side nexora">'
            '<div class="bench-side-kicker">NEXORA RUNTIME</div>'
            '<div class="bench-side-title">AI + NEXORA</div>'
            '<div class="bench-side-flow">AI gets current step → NEXORA forecasts next dependency → reserves → allocates → continues</div>'
            '<div class="bench-side-detail">The underlying AI is unchanged; NEXORA changes the runtime coordination around it.</div>'
            '</div>', unsafe_allow_html=True
        )

    st.markdown("### 05 · Trial-by-trial results")
    table_rows = []
    for row in rows:
        table_rows.append({
            "Trial": row["trial"],
            "AI workflow": " → ".join(row["workflow"]),
            "Contention": row["contention"],
            "Reactive AI (s)": round(row["ai_total"], 4),
            "AI + NEXORA (s)": round(row["nexora_total"], 4),
            "AI wait (s)": round(row["ai_wait"], 4),
            "NEXORA wait (s)": round(row["nexora_wait"], 4),
        })
    st.dataframe(table_rows, use_container_width=True, hide_index=True)

    st.markdown("### 06 · What changed between the two runs")
    trace_left, trace_right = st.columns(2)
    with trace_left:
        st.markdown('<div class="bench-trace-title">REACTIVE AI TRACE</div>', unsafe_allow_html=True)
        st.code("\n".join(first["ai_trace"]), language="text")
    with trace_right:
        st.markdown('<div class="bench-trace-title">AI + NEXORA TRACE</div>', unsafe_allow_html=True)
        st.code("\n".join(first["nexora_trace"]), language="text")

    with st.expander("AI planning response used for this benchmark"):
        st.code(first["raw_ai"], language="json")

    with st.expander("Benchmark methodology"):
        st.write(
            "Every benchmark starts from the problem statement currently shown above. "
            "Ollama generates the service workflow without being given a fixed workflow. "
            "That workflow is reused unchanged for the two runtime policies. The reactive policy "
            "requests the contention resource only when its step is reached. NEXORA reserves the "
            "same future dependency before the competing workflow can claim it. Service durations "
            "are controlled benchmark workload costs, while the planning/workflow itself comes from "
            "the real local AI model. Timings are measured with perf_counter() and refreshed on every run."
        )
