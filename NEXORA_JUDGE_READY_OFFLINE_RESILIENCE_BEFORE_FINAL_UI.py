...
def send_nexora_otp(recipient, otp):
    ...
    msg.set_content(
        ...
        f"If you did not request this login, you can ignore this email.\n\n"
        f"— NEXORA Security"
        """
    )
...
def auth_css():
    ...
    .auth-page{max-width:460px;margin:6vh auto 0;padding:0 18px}
    .auth-shell{position:relative;background:rgba(8,13,30,.96);border:1px solid rgba(111,151,255,.22);border-radius:18px;padding:34px 34px 28px;box-shadow:0 24px 70px rgba(0,0,0,.35),0 0 40px rgba(47,111,255,.08);overflow:hidden}
    .auth-shell:before{content:"";position:absolute;left:0;right:0;top:0;height:2px;background:linear-gradient(90deg,transparent,#39d5ff 35%,#8d65ff 65%,transparent);opacity:.9}
    .auth-logo{text-align:center;color:#f4f7ff;font-family:Orbitron,sans-serif;font-size:25px;font-weight:900;letter-spacing:3px;line-height:1.1}
    .auth-logo span{color:#39d5ff}
    .auth-product{text-align:center;margin-top:7px;color:#7185aa;font-size:9px;letter-spacing:2.2px;text-transform:uppercase}
    .auth-title{text-align:center;color:#f4f7ff;font-size:23px;font-weight:700;margin:28px 0 7px}
    .auth-subtitle{text-align:center;color:#7f91b3;font-size:11px;line-height:1.6;margin:0 auto 24px;max-width:340px}
    .auth-steps{display:flex;align-items:center;justify-content:center;gap:7px;margin:0 auto 25px}
    .auth-step-item{display:flex;align-items:center;gap:6px;color:#637696;font-size:8px;letter-spacing:1px;text-transform:uppercase;white-space:nowrap}
    .auth-step-dot{width:19px;height:19px;border-radius:50%;display:flex;align-items:center;justify-content:center;border:1px solid rgba(111,151,255,.22);background:rgba(255,255,255,.025);font-size:8px}
    .auth-step-item.active{color:#dff9ff}
    .auth-step-item.active .auth-step-dot{border-color:rgba(57,213,255,.7);background:rgba(57,213,255,.1);color:#39d5ff;box-shadow:0 0 14px rgba(57,213,255,.12)}
    .auth-step-line{width:24px;height:1px;background:rgba(111,151,255,.18)}
    .auth-form-label{color:#91a4c4;font-size:10px;font-weight:600;letter-spacing:.3px;margin:0 0 5px}
    .auth-info{display:flex;align-items:flex-start;gap:9px;padding:11px 12px;border:1px solid rgba(111,151,255,.15);background:rgba(70,105,190,.055);border-radius:10px;color:#8295b6;font-size:9px;line-height:1.55;margin-top:15px}
    .auth-info-icon{color:#39d5ff;font-weight:800;font-size:11px}
    .auth-email-box{text-align:center;padding:12px 14px;margin:0 0 18px;border:1px solid rgba(57,213,255,.16);background:rgba(57,213,255,.045);border-radius:10px;color:#8295b6;font-size:10px}
    .auth-email{display:block;color:#39d5ff;font-size:12px;font-weight:700;margin-top:3px;word-break:break-all}
    .auth-footer{text-align:center;color:#596b8b;font-size:9px;margin-top:20px;letter-spacing:.2px}
    .auth-security{display:flex;justify-content:center;gap:16px;margin-top:16px;color:#647796;font-size:8px;letter-spacing:.6px;text-transform:uppercase}
    .auth-security span:before{content:"✓";color:#43ddb0;margin-right:4px}
    .auth-error{margin-top:12px}
    /* Streamlit input cleanup inside auth card */
    .auth-shell [data-testid="stTextInput"] label{color:#91a4c4!important;font-size:10px!important;font-weight:600!important}
    .auth-shell [data-testid="stTextInput"] input{height:43px!important;border-radius:9px!important;background:rgba(255,255,255,.035)!important;border:1px solid rgba(130,158,210,.18)!important;color:#edf4ff!important;font-size:13px!important}
    .auth-shell [data-testid="stTextInput"] input:focus{border-color:rgba(57,213,255,.65)!important;box-shadow:0 0 0 1px rgba(57,213,255,.12),0 0 18px rgba(57,213,255,.07)!important}
    .auth-shell [data-testid="stTextInput"]{margin-bottom:8px!important}
    .auth-shell .stButton>button,.auth-shell [data-testid="stFormSubmitButton"] button{height:43px!important;border-radius:9px!important;border:1px solid rgba(57,213,255,.42)!important;background:linear-gradient(135deg,rgba(28,151,194,.95),rgba(77,73,186,.95))!important;color:#fff!important;font-size:10px!important;font-weight:800!important;letter-spacing:1px!important;box-shadow:0 8px 24px rgba(38,110,200,.14)!important}
    .auth-shell .stButton>button:hover,.auth-shell [data-testid="stFormSubmitButton"] button:hover{border-color:rgba(57,213,255,.8)!important;transform:translateY(-1px)}
    .auth-shell .stButton>button[kind="secondary"]{background:rgba(255,255,255,.025)!important;border-color:rgba(111,151,255,.16)!important;color:#91a4c4!important;box-shadow:none!important}
    .auth-shell .stForm{padding:0!important;border:0!important;background:transparent!important}
    .auth-shell [data-testid="stFormSubmitButton"]{margin-top:8px!important}
    .auth-shell [data-testid="stMarkdownContainer"] p{margin-bottom:0}
    @media (max-width:600px){.auth-page{margin-top:3vh;padding:0 10px}.auth-shell{padding:28px 22px 24px}.auth-steps{gap:4px}.auth-step-line{width:13px}.auth-step-item{font-size:7px}}
    </style>
    """, unsafe_allow_html=True)


def _auth_steps(active):
    labels = [("1", "Identity", "login"), ("2", "Email OTP", "otp"), ("3", "Workspace", "workspace")]
    parts = []
    for index, (num, label, key) in enumerate(labels):
        active_class = " active" if key == active else ""
        parts.append(f'<div class="auth-step-item{active_class}"><span class="auth-step-dot">{num}</span>{label}</div>')
        if index < len(labels) - 1:
            parts.append('<div class="auth-step-line"></div>')
    st.markdown('<div class="auth-steps">' + ''.join(parts) + '</div>', unsafe_allow_html=True)


def _auth_footer():
    st.markdown(
        '<div class="auth-security"><span>Encrypted session</span><span>Email verified</span><span>OTP protected</span></div>'
        '<div class="auth-footer">NEXORA Predictive Runtime Orchestration</div>',
        unsafe_allow_html=True,
    )
...
def render_auth_gate():
    ...
    st.markdown('<div class="auth-page"><div class="auth-shell">', unsafe_allow_html=True)
    st.markdown('<div class="auth-logo">NEXORA<span>•</span>SECURE</div>', unsafe_allow_html=True)
    st.markdown('<div class="auth-product">Command Center Access</div>', unsafe_allow_html=True)

    if mode == "login":
        _auth_steps("login")
        st.markdown('<div class="auth-title">Welcome back</div>', unsafe_allow_html=True)
        st.markdown('<div class="auth-subtitle">Sign in to access your NEXORA Command Center. A one-time email code will be required after your password is verified.</div>', unsafe_allow_html=True)
        ...
        if submitted:
            ...
            if user is None or not verify_password(password, user["password_hash"]):
                ...
                try:
                    ...
                    st.error(str(exc))
        st.markdown('<div class="auth-info"><span class="auth-info-icon">✓</span><span>Your password is verified locally. Access is granted only after the OTP sent to your registered email is verified.</span></div>', unsafe_allow_html=True)
        if st.button("CREATE NEXORA ACCOUNT", use_container_width=True):
        ...
        st.markdown('<div class="auth-subtitle">Create a NEXORA identity using your real email address. We will send an OTP to verify it before access is granted.</div>', unsafe_allow_html=True)
        ...
        _auth_steps("otp")
        st.markdown('<div class="auth-title">Verify your identity</div>', unsafe_allow_html=True)
        st.markdown('<div class="auth-subtitle">Enter the six-digit code we sent to your registered email address.</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="auth-email-box">Verification code sent to<span class="auth-email">{esc(email)}</span></div>', unsafe_allow_html=True)
        with st.form("nexora_otp_form"):
            code = st.text_input("6-digit verification code", max_chars=6, placeholder="123456")
            submitted = st.form_submit_button("VERIFY & OPEN COMMAND CENTER", use_container_width=True)
        if submitted:
            if not user_id:
                st.session_state.auth_mode = "login"
                st.rerun()
        ...
        if remaining == 0:
            ...
            st.markdown(f'<div style="text-align:center;color:#657896;font-size:9px;margin-top:12px">Resend available in {remaining}s</div>', unsafe_allow_html=True)
